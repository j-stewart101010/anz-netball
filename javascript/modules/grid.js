...
define([
  ...
  var Grid = function (w, h) {
    ...
    this.zoom = 0.2;
    this.defaultZoom =1;//canvas.width/(15*this.squareWidth);//0.6 + (canvas.width / 10000);
    
    // this.camera.x=(canvas.width*0.5*(1-this.defaultZoom))/this.defaultZoom;
    // this.camera.y=(canvas.height*0.5*(1-this.defaultZoom))/this.defaultZoom;
    ...
    for(var i=0;i<model.content.length;i++) {
      switch(model.content[i].tiletype) {
        ...
        case "image":
          ...
          model.content[i].backbox.addBox(new Box(this.offScreenCtx, model.content[i].storyimage, {top:26.5,left:5,width:13,height:13,padding:0,contentType:"image"}));
        ...
        case "video":
          ...
          model.content[i].box.addBox(new Box(this.offScreenCtx, "VIDEO", {contentType:"text",fontStyle:"bold",left:8.5,top:10,width:80,height:25,padding:2,fontSize:9,backgroundColour:"rgba(0,0,0,0.35)",textunderlay:"fit"}));
          model.content[i].box.calculate();
          model.content[i].box.addBox(new Box(this.offScreenCtx, model.content[i].textname, {contentType:"text",left:model.content[i].box.last().right,top:10,width:80,height:25,padding:2,fontSize:9,backgroundColour:"rgba(0,0,0,0.35)",textunderlay:"fit"}));
          model.content[i].box.addBox(new Box(this.offScreenCtx, model.content[i].textsubject, {contentType:"text",left:8.5,top:20,width:80,height:20,padding:2,fontSize:22,backgroundColour:"rgba(0,0,0,0.35)",textunderlay:"fit"}));
  ...
  //use the difference to move the camera when holding
    ...
    for(i=0; i<model.content.length; i++) {
      ...
      x+=model.content[i].position.x*this.squareWidth;
      wtx+=model.content[i].position.x*this.squareWidth;
      ...
      y+=model.content[i].position.y*this.squareHeight;
      wty+=model.content[i].position.y*this.squareHeight;
      ...
      //collision with screen. whether it's worth drawing or not
          ...
          if(Config.mouse.x>=repeatx && Config.mouse.x<(repeatx+model.content[i].scale*this.squareWidth*this.zoom) && (Config.mouse.y-this.canvasOffset)>=repeaty && (Config.mouse.y-this.canvasOffset)<(repeaty+model.content[i].scale*this.squareHeight*this.zoom)) {
            ...
            ctx.fillRect(repeatx,repeaty,model.content[i].scale*this.squareWidth*this.zoom,model.content[i].scale*this.squareHeight*this.zoom);
          ...
          ctx.fillStyle="black";
          ctx.fillText(Math.floor(repeatwtx)+","+Math.floor(repeatwty),repeatx,repeaty);
          //Math.ceil(canvas.height/(totalWorldHeight*this.zoom*2))
...
////  SECOND PASS: SCALED/SCALING WHILE UNFLIPPED/FULLY-FLIPPED TILES ONLY
    for(i=0; i<model.content.length; i++) {
      ...
      x%=totalWorldWidth;
      x-=totalWorldWidth*Math.ceil(canvas.width/(totalWorldWidth*this.zoom*2));
      wtx-=totalWorldWidth*Math.ceil(canvas.width/(totalWorldWidth*this.zoom*2));
      x+=model.content[i].position.x*this.squareWidth;
      wtx+=model.content[i].position.x*this.squareWidth
      ...
      //    if(repeatx>(-model.content[i].scale*this.squareWidth*this.zoom) && repeatx<canvas.width && repeaty>(-model.content[i].scale*this.squareHeight*this.zoom) && repeaty<canvas.height){
          ...
          if(model.content[i].scaleProgress>0 && model.content[i].actionX==repeatwtx && model.content[i].actionY==repeatwty) {
    ...
    for(i=0; i<model.content.length; i++) {
      ...
      //    if(repeatx>(-model.content[i].scale*this.squareWidth*this.zoom) && repeatx<canvas.width && repeaty>(-model.content[i].scale*this.squareHeight*this.zoom) && repeaty<canvas.height){
            //what to draw depending on data
          if(model.content[i].actionX==repeatwtx && model.content[i].actionY==repeatwty && model.content[i].flipProgress>0 && model.content[i].flipProgress<1){
            if(model.content[i].flipProgress<0.5) {
              //render the front side to offscreen buffer
              drawAngle=model.content[i].flipProgress*180; //0-90 degrees
              model.content[i].box.render(this.offScreenCtx, 0, 0, model.content[i].scale);

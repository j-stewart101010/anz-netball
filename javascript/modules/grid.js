...
define([
    ...
    for(var i=0;i<TileData.content.length;i++) {
      switch(TileData.content[i].tiletype) {
        ...
        case "video":
          ...
          TileData.content[i].box.addBox(new Box(this.offScreenCtx, "VIDEO", {contentType:"text",fontStyle:"bold",left:8.5,top:10,width:80,height:25,padding:2,fontSize:9,backgroundColour:"70,145,185",backgroundOpacity:0.11,textunderlay:"fit"}));
          TileData.content[i].box.calculate();
          TileData.content[i].box.addBox(new Box(this.offScreenCtx, TileData.content[i].textname, {contentType:"text",left:TileData.content[i].box.last().right,top:10,width:80,height:25,padding:2,fontSize:9,backgroundColour:"0,0,0",backgroundOpacity:0.35,textunderlay:"fit"}));
  ...
  //use the difference to move the camera when holding
    ...
    for(i=0;i<this.interactingTiles.length;i++) {
      if(this.interactingTiles[i].removeCounter>=12) {
    ...
    // var foundTile=-1;
    // for(var i=0;i<this.interactingTiles.length;i++) {
    //   //if(this.interactingTiles[i].
    // };
...
////  SECOND PASS: interactive Tiles
    for(i=0; i<TileData.content.length; i++) {
      ...
      //    if(repeatx>(-TileData.content[i].scale*this.squareWidth*this.zoom) && repeatx<canvas.width && repeaty>(-TileData.content[i].scale*this.squareHeight*this.zoom) && repeaty<canvas.height){
          for(j=0;j<this.interactingTiles.length;j++) {
            if(this.interactingTiles[j].modelIndex==i && this.interactingTiles[j].worldX==repeatwtx && this.interactingTiles[j].worldY==repeatwty) {
